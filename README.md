...
LGPL-3.0-only. Its exact upstream commit/tree, content integrity, attribution,
and license text live in `truenas-app/vendor/truenas-apps/`.
...
| npm packages | As verified 2026-08-06, npm `latest` is `0.24.4` for all four packages, published from tag `v0.24.4`; exact `0.24.3` packages do not exist and cannot be produced. npm `next` is `0.25.0-rc.9`. | npm is one patch ahead of the fleet by design: `0.24.4` is `v0.24.3` plus the poker-usage smoke fix that `v0.24.3` predates. Do not present npm as a `0.24.3` installation path, and do not "correct" the fleet to match — see [npm is one patch ahead](./docs/STABLE-0.24.3.md#npm-is-one-patch-ahead-of-the-fleet). |

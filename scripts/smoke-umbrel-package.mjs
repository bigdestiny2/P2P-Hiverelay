...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`Usage: node scripts/smoke-umbrel-package.mjs [options]

Options:
  --compose <path>       Umbrel docker-compose.yml path (default: umbrel-app/docker-compose.yml)
  --image-ref <ref>      Runtime image override for local testing; static compose checks still run
  --timeout-ms <ms>      Container startup timeout (default: 120000)
  --evidence <path>      Write public-safe smoke evidence JSON on success
  --keep-data            Do not remove the temporary /data directory after the run

Statically validates the Umbrel package safety contract, boots the packaged
image with an Umbrel-like APP_SEED and /data bind mount, verifies dashboard,
setup, wallet, and service-management APIs, restarts the container, and checks
identity + wallet/service-selection persistence.
`)
  process.exit(0)
}

const composePath = path.resolve(readFlag('--compose') || path.join(repoRoot, 'umbrel-app', 'docker-compose.yml'))
const timeoutMs = parseTimeoutMs(readFlag('--timeout-ms') || process.env.HIVERELAY_SMOKE_TIMEOUT_MS || '120000')
const imageOverride = readFlag('--image-ref') || ''
const evidenceFile = readFlag('--evidence') || process.env.HIVERELAY_UMBREL_SMOKE_EVIDENCE || ''
...
if (imageOverride && !isSafeImageRef(imageOverride)) fatal('--image-ref contains unsupported characters')

const compose = readFileSync(composePath, 'utf8')
const packageInfo = validateCompose(compose, composePath)
const imageRef = imageOverride || packageInfo.imageRef
if (!isSafeImageRef(imageRef)) fatal('package image reference contains unsupported characters')
if (evidenceFile && !isDigestPinnedImageRef(imageRef)) {
  fatal('Umbrel package smoke evidence requires a GHCR semver tag plus sha256 digest image ref')
}
if (evidenceFile) requireImageManifestEvidence(imageManifestEvidenceFile, imageRef)

const dataDir = await mkdtemp(path.join(tmpdir(), 'hiverelay-umbrel-smoke-'))
const checks = []
let cleaned = false
let failed = false
let first = null
...
try {
  recordCheck('composeSafety', {
    composePath: path.relative(repoRoot, composePath),
    packageImageRef: packageInfo.imageRef
  })

  first = await bootAndVerify('first', dataDir, true)
  firstPublicKey = first.publicKey
  recordCheck('firstBoot', {
    dashboard: true,
    setup: true,
    serviceCatalog: true,
  ...
  recordCheck('dashboardWebSocket', first.dashboardWebSocket)
  recordCheck('acceptModeDefault', { mode: first.acceptMode })
  const usageTelemetry = await assertUsageTelemetry(first.baseUrl, first.token)
  recordCheck('usageTelemetry', usageTelemetry)
  await setWallet(first.baseUrl, first.token, 'operator@example.com')
  recordCheck('walletWrite', { destinationSaved: true })
  await setServices(first.baseUrl, first.token)
  recordCheck('servicesSave', { restartRequired: true, plugins: EXPECTED_POKER_AI_PLUGINS })
  await delay(1000)
  await first.stop()
  first = null

  second = await bootAndVerify('second', dataDir)
  recordCheck('secondBoot', {
    dashboard: true,
    setup: true,
    serviceCatalog: true,
    dashboardUiHardening: true,
  ...
  if (second.publicKey !== firstPublicKey) {
    throw new Error('relay public key changed across Umbrel-style restart')
  }
  recordCheck('identityPersistence', { publicKeyStable: true })
  await assertWallet(second.baseUrl, second.token, 'operator@example.com')
  recordCheck('walletPersistence', { destinationPersisted: true })
  const servicePersistence = await assertServices(second.baseUrl, second.token)
  recordCheck('servicesPersistence', { selectedServicesActive: true, ...servicePersistence })
  await second.stop()
  second = null

  writeSmokeEvidence()
  console.log(`Umbrel package smoke passed: ${imageRef}`)
} catch (err) {
  failed = true
...
function validateCompose (text, file) {
  const appDataDir = '$' + '{APP_DATA_DIR}'
  const appSeed = '$' + '{APP_SEED}'
  const image = text.match(/^\s*image:\s*(\S+)\s*$/m)?.[1]
  if (!image) fatal(`Could not find web image in ${file}`)
  if (!/^ghcr\.io\/bigdestiny2\/p2p-hiverelay:\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?@sha256:[a-f0-9]{64}$/i.test(image)) {
    fatal('Umbrel package image must be a tag plus multi-arch sha256 digest pin')
  }
  assertIncludes(text, 'APP_HOST: blindspark_web_1', 'app_proxy APP_HOST')
  assertIncludes(text, 'APP_PORT: 9100', 'app_proxy APP_PORT')
  assertIncludes(text, appDataDir + '/data:/data', 'persistent /data bind mount')
  assertIncludes(text, 'HIVERELAY_UI_SIMPLE: "true"', 'simple appliance UI env')
  assertIncludes(text, 'HIVERELAY_UI_EXPOSE_TOKEN: "true"', 'proxy auth token env')
  assertIncludes(text, 'HIVERELAY_ACCEPT_MODE: review', 'home-server review accept-mode env')
  assertIncludes(text, 'HIVERELAY_MAX_STORAGE: 10GB', 'home-server storage cap env')
  assertIncludes(text, 'APP_SEED: ' + appSeed, 'Umbrel APP_SEED env')

  if (/^\s+ports:\s*$/m.test(text)) fatal('Umbrel package must not publish raw host ports')
  if (/^\s+privileged:\s*true\s*$/m.test(text)) fatal('Umbrel package must not use privileged mode')
  if (/^\s+network_mode:\s*host\s*$/m.test(text)) fatal('Umbrel package must not use host networking')
  if (text.includes('/var/run/docker.sock')) fatal('Umbrel package must not mount the Docker socket')

  return { imageRef: image }
}

async function bootAndVerify (label, volumeDir, verifyDashboardWebSocket = false) {
  const containerName = `hiverelay-umbrel-smoke-${label}-${process.pid}-${Date.now()}`
  ...
  await run('docker', [
    ...
    '--name', containerName,
    '-v', `${volumeDir}:/data`,
    '-e', 'APP_SEED=umbrel-smoke-seed-000000000000000000000000000000000',
    '-e', 'HOME=/data',
  ...
  try {
    ...
    const overview = await waitForOverview(baseUrl, timeoutMs)
    ...
    const dashboardUiHardening = assertDashboardUiHardening(dashboard)

    const setup = await fetchText(`${baseUrl}/wizard?edit=1`)
    assertIncludes(setup, 'Setup', 'setup wizard')
    const setupUiHardening = assertSetupWizardUiHardening(setup)
    ...
    const wizard = await fetchJson(`${baseUrl}/api/wizard`, auth)
    if (!wizard || typeof wizard !== 'object' || !Object.prototype.hasOwnProperty.call(wizard, 'isComplete')) {
      throw new Error('wizard API did not return setup state')
    }

    const services = await fetchJson(`${baseUrl}/api/manage/services/available`, auth)
    if (!Array.isArray(services.available) || !services.available.includes('ai') || !services.available.includes('poker')) {
      throw new Error('service catalog is missing built-in services')
    }
    if (!services.bundles || !Array.isArray(services.bundles.poker)) {
      throw new Error('service catalog is missing poker provider bundle')
    }
    ...
    const dashboardWebSocket = verifyDashboardWebSocket
      ? await assertDashboardWebSocket(baseUrl, token)
      : null
    return { baseUrl, token, publicKey: overview.publicKey, acceptMode, dashboardUiHardening, setupUiHardening, dashboardWebSocket, healthVersion: health.version || '', stop }
  } catch (err) {
    try {
...
async function assertAcceptModeDefault (baseUrl) {
  ...
  if (catalog?.acceptMode !== 'review') {
    throw new Error(`Umbrel package did not honor HIVERELAY_ACCEPT_MODE=review: got ${JSON.stringify(catalog?.acceptMode)}`)
  }
...
async function setWallet (baseUrl, token, destination) {
  ...
  if (value !== destination) throw new Error('wallet destination did not save through Umbrel package API')
}

async function assertWallet (baseUrl, token, destination) {
  const out = await fetchJson(`${baseUrl}/api/subsidy`, {
    Authorization: `Bearer ${token}`
  })
  const value = out?.payoutDestination?.value || ''
  if (value !== destination) throw new Error('wallet destination did not persist across Umbrel-style restart')
}
...
async function setServices (baseUrl, token) {
  ...
  assertPluginList(out?.config?.plugins, EXPECTED_POKER_AI_PLUGINS, 'service selection did not save through Umbrel package API')
  if (out?.restartRequired !== true) throw new Error('service config save did not report restartRequired:true')
}

async function assertServices (baseUrl, token) {
  const out = await fetchJson(`${baseUrl}/api/manage/services/available`, {
    Authorization: `Bearer ${token}`
  })
  if (out?.enabled !== true) throw new Error('service selection did not persist enabled:true across Umbrel-style restart')
  assertPluginList(out?.plugins, EXPECTED_POKER_AI_PLUGINS, 'service selection did not persist across Umbrel-style restart')
  assertPluginListContains(out?.active, EXPECTED_POKER_AI_PLUGINS, 'selected services did not start after Umbrel-style restart')
  return {
    plugins: out.plugins,
    active: EXPECTED_POKER_AI_PLUGINS.filter(name => out.active.includes(name))
  }
}
...
function assertPluginListContains (actual, expected, label) {
  if (!Array.isArray(actual) || expected.some(name => !actual.includes(name))) {
    throw new Error(`${label}: got ${JSON.stringify(actual)}`)
  }
}
...
async function waitForHealth (baseUrl, timeout) {
  return waitForJson(`${baseUrl}/health`, timeout, (json) => json && json.running === true)
}

async function waitForOverview (baseUrl, timeout) {
  return waitForJson(`${baseUrl}/api/overview`, timeout, (json) => json && /^[a-f0-9]{64}$/i.test(json.publicKey || ''))
}

async function waitForJson (url, timeout, predicate) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      lastError = new Error(`${url} returned an unexpected response`)
    } catch (err) {
      lastError = err
    }
    await delay(1500)
...
function writeSmokeEvidence () {
  ...
  if (!image) fatal('Umbrel package smoke evidence requires a GHCR semver tag plus sha256 digest image ref')
  const body = {
    schemaVersion: 1,
  ...
  assertPublicSafeSmoke(body, 'Umbrel package smoke')
...
async function cleanup () {
  ...
  cleaned = true
  if (keepData) {
    console.log(`Keeping Umbrel smoke data directory: ${dataDir}`)
    return
  }
  try {

...
# A heredoc-generated file (`RUN cat > … <<'EOS'`) is silently dropped by some
# remote builders (e.g. Fly.io's remote builder), leaving the image with a
# missing /usr/local/bin/docker-entrypoint.sh → container exits 127 on boot.
# COPYing a real file from the build context is robust across all builders.

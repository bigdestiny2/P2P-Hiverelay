...
export const STORE_FORMAT_AUTHORITY_V1 = Object.freeze([
  entry('artifact.encoding', 'magic[8]=ASCII(HRBSFA01) || authorityVersion:u16be=1 || formatMajor:u16be=1 || formatMinor:u16be=2 || schemaCatalogLength:u64be || exact schemaCatalog bytes || entryCount:u16be || sorted entries(nameLength:u16be || printable ASCII name || valueLength:u32be || printable ASCII value)'),
  ...
  entry('wal.cell.put-atomic-committed', 'Cell recordType=17 payload is canonical BlindPutAtomicCommittedStoreV1 with required canonical profile-1 resultBindingBytes; the BlindWalHeaderV2 exclusively binds transactionId, virtualBucket, mapGeneration, and writerFenceTokenHash; one frame reconstructs the complete committed spend, commitment index, Cell record, exact result, and derived stored/control/tombstone accounting with no reservation deadline, attempt, reserved-epoch, or caller-supplied accounting fields; Cell record types 1..16 and checkpoint subtypes 1..4 remain byte-for-byte legacy authorities')
])

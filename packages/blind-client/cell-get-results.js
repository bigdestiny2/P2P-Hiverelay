...
import { getCellResultV1 } from '@hiverelay/blind-protocol/schemas'
import { asBytes } from './bytes.js'
import { openCell } from './cells.js'
import { fail } from './errors.js'
import { decodeCanonicalCopy, sameBytes } from './signed.js'
...
export class VerifiedCellGetResult {
  ...
  get familyId () { return FAMILY.CELL }
  get operationId () { return OPERATION.CELL.GET }
...
export function verifyCellGetResult (options) {
  ...
  const requestCommitment = b4a.from(asBytes(
    options.requestCommitment, 'requestCommitment', 32))
...
// Keep the established adapter name while making the boundary strictly
// narrower: this verifier rejects every operation except CELL.GET.
export const verifyOperationResult = verifyCellGetResult
...
export async function openVerifiedCellGetResult (options) {
  ...
  if (!internal) {
    fail('BAD_CLIENT_INPUT', 'a package-owned VerifiedCellGetResult is required')
  }
  ...
  const relayPublicKey = b4a.from(asBytes(
    readCap.relayPublicKey, 'readCap relayPublicKey', 32))

...
import { assertSafeOnionVports, selectOnionReadPort } from '../../transports/tor/vport-policy.js'
  ...
  async _startLifecycle () {
    ...
    try {
      ...
      if (this.config.transports && this.config.transports.tor) {
        ...
        assertSafeOnionVports(torOpts.vports, {
          apiPort: this.api ? this.api.port : null,
          trustedProxyGatewayPort: this.gatewayServer && this.gatewayServer.trustProxy
            ? this.gatewayServer.port
            : null
        })
        ...
        // Onion HTTP must terminate on the dedicated read-only gateway. Tor
        // daemon forwarding arrives over loopback, so pointing the default
        // vport at RelayAPI would inherit its localhost management fallback.
        const torReadPort = selectOnionReadPort(this.gatewayServer)
        this.torTransport = new TorTransport({
          ...
          localPort: torReadPort,
          readVport: torReadPort == null ? null : 80,
          peerListener,
        ...
        this.torTransport.on('hidden-service', ({ onionAddress }) => {
          if (torReadPort != null && typeof this.gatewayServer.addCompatibilityHost === 'function') {
            this.gatewayServer.addCompatibilityHost(onionAddress)
          }

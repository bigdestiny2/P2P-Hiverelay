 ...
 *   - GET /.well-known/hiverelay.json — signed relay capabilities
...
import { buildCapabilityRoutePayload } from './api-capabilities.js'
...
export class GatewayServer extends EventEmitter {
  ...
  addCompatibilityHost (value) {
    const normalized = normalizeCompatibilityHost(value)
    if (!normalized) throw new Error('compatibility host must be a bounded canonical host')
    if (!this.compatibilityHosts.has(normalized) && this.compatibilityHosts.size >= MAX_HOST_ALLOWLIST_ENTRIES) {
      throw new Error('gateway compatibility host limit reached')
    }
    this.compatibilityHosts.add(normalized)
    return normalized
  }
  ...
  async _handle (req, res) {
    ...
    try {
      ...
      // Signed public capability document. Keeping this on the read-only
      // server lets onion clients discover the relay without ever reaching
      // RelayAPI's management route table.
      if (req.method === 'GET' && path === '/.well-known/hiverelay.json') {
        const version = this.node.api && typeof this.node.api._relayVersion === 'function'
          ? this.node.api._relayVersion()
          : null
        ...
        writeJson(res, result.payload, result.status || 200, result.headers || null)
        return
      }

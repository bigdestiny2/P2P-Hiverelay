...
import { classifyOnionVports } from '../transports/tor/vport-policy.js'
import {
...
const ONION_ENDPOINT_ADVERTISEMENT_LEASE_MS = 90 * 24 * 60 * 60 * 1000
 ...
 */
function buildPrivacyTransports ({ relay, config, custodyEnabled, advertisedAtMs }) {
  ...
  const classified = classifyOnionVports(vports, {
    peerVport: hasPeerListener && Number.isSafeInteger(tt.peerVport) ? tt.peerVport : null,
    peerTargetPort: hasPeerListener && Number.isSafeInteger(tt.peerListener.port)
      ? tt.peerListener.port
      : null,
    readVport: tt.readVport,
    readTargetPort: Number.isSafeInteger(tt.localPort) ? tt.localPort : null
  })
  const vportRoles = {
    ...(classified.readPlane == null ? {} : { readPlane: classified.readPlane }),
    ...(classified.peer == null ? {} : { peer: classified.peer })
  }
  const supports = []
  ...
  if (classified.peer != null && relay && relay._publishProtocol) supports.push('seed.publish')
  ...
  return [{
    ...
    addresses: [{
      ...
      notBefore,
      // This is a rolling, signed advertisement lease rather than an
      // unimplemented endpoint-key rotation promise. Fresh capability docs
      // never continue publishing an already-expired onion endpoint.
      notAfter: advertisedAtMs + ONION_ENDPOINT_ADVERTISEMENT_LEASE_MS,
      priority: 10
    }],
...
export function buildCapabilityDoc (opts = {}) {
  ...
  const privacyTransports = buildPrivacyTransports({
    relay,
    config,
    custodyEnabled,
    advertisedAtMs: attestedAt
  })

...
export class HyperGateway extends EventEmitter {
  ...
  async handle (req, res, context = null) {
    ...
    // Resolve the target drive + file path. An issued exact-app context
    // (only the isolation-validated GatewayServer may mint one) routes an
    // app-origin Host directly to its seeded drive without a /v1/hyper/KEY
    // path prefix; everything else uses the public key-path form.
    let keyHex
    let filePath
    let byteMode = 'generated'
    if (isIssuedExactAppContext(context)) {
      keyHex = context.appKey
      filePath = context.path || '/'
      byteMode = context.byteMode || 'exact'
    } else {
    ...
    try {
      ...
      // Exact app-origin serving charges the response body against the
      ...
      if (!isHead && typeof context.reserveResponseBytes === 'function' &&
        ...
        // response), so it advertises the generated byte mode.
        sendJson({ error: 'Gateway byte-rate limit exceeded' }, 429, { 'Retry-After': String(retryAfter), 'X-Hive-Byte-Mode': 'generated' })
        return
      }
      ...
      // Exact app-origin serving advertises its own app key and byte mode so
      // clients can distinguish an exact-origin response from a key-path one.
      if (isIssuedExactAppContext(context)) {
        res.setHeader('X-Hive-App-Key', keyHex)
        res.setHeader('X-Hive-Byte-Mode', byteMode)
      }
      ...
      if (contentType.includes('text/html')) {
        ...
        // Base-URL rewriting is only needed for the public key-path read plane
        // (/v1/hyper/KEY/...), where absolute paths must become relative so a
        // Vite-built app resolves assets under the key prefix. Exact app-origin
        // serving (Host: app.suffix) already mounts the app at its own root, so
        // absolute paths are correct and must be preserved byte-for-byte.
        if (!isIssuedExactAppContext(context)) {

...
- [x] **4.142** Umbrel setup wizard error text bounds — the setup wizard now applies the same bounded, control-character-free error text to API failures, inline field errors, load-error replacement content, and delegated action status, with unit and packaged-smoke guards so setup cannot regress to huge or malformed backend messages in appliance UI.
- [x] **4.143** Ecosystem latest-default preparation path — `ecosystem:prepare-latest` now lets PearBrowser, PearPaste, anonGPT, and the other tracked app consumers move their manifest defaults/source markers to npm `latest` before npm dist-tags are promoted, while deliberately skipping lockfile refresh so strict post-publish sync/audit remains the release gate for real registry metadata.
- [x] **4.144** Ecosystem explicit npm latest release gate — `npm run release:check-npm-latest` now gives operators a read-only, package-by-package proof that the four-package HiveRelay npm line has promoted `latest` to the current release before app lockfiles, live consumers, fleet metadata, or store submissions are treated as publish-ready. The command distinguishes stale verified tags from unverified registry access and supports `-- --json` for release evidence.
- [x] **4.145** Ecosystem app latest-default guardrails — PearBrowser, PearPaste, anonGPT, p2pbuilders, Opengit, Pear POS, Pear Tickets, and the loose HiveRelay test consumer now have source/test/validator/docs updated or guarded so the app default is npm `latest`, while stale local lockfile records remain explicit warnings until npm `latest` is promoted to `0.20.2`.
- [x] **4.146** Official Umbrel PR handoff gate — `npm run release:check-official-umbrel-pr` now fails local release handoff while the official `umbrel-app/umbrel-app.yml` submission URL is still `PENDING`, allows that placeholder only for first pre-PR export with empty release notes, rejects unsafe symlinked manifests, and passes only after a real `getumbrel/umbrel-apps` PR URL is stamped.
- [x] **4.147** Packaged dynamic payout proof — release-image and Umbrel-package smoke now require packaged dashboard HTML to render payout copy/edit controls as DOM-created non-submit buttons with no payout `innerHTML` assignment, and release/final handoff verifiers reject smoke sidecars unless `dynamicPayoutControls` is true. This carries the Umbrel add-wallet no-op regression guard from unit tests into release evidence.
- [x] **4.148** Manual dynamic payout review proof — `umbrel:write-runtime-review`, `umbrel:verify-runtime-review`, and final handoff verification now require the real-device Umbrel runtime-review sidecar to include `dynamicPayoutControlsObserved`, so the final reviewer proof explicitly covers payout Add/Change/copy controls behaving as buttons without page refresh.
- [x] **4.149** Npm package publish metadata proof — all four publishable workspaces now physically include their declared `README.md` and Apache-2.0 `LICENSE` files, and `npm run release:check-npm-packages` runs `npm pack --dry-run --json --workspace <workspace>` with an isolated npm cache to confirm the tarballs include README/license metadata with no obvious unsafe paths such as nested dependencies, docs/tests, env files, or key material. The release workflow now runs this gate before npm publish, so tarball-shape regressions fail before registry side effects.
- [x] **4.150** Ecosystem prepared-latest check mode — `npm run ecosystem:prepare-latest -- --check` now verifies that PearBrowser, PearPaste, anonGPT, p2pbuilders, Opengit, Pear POS, Pear Tickets, and the local Hiverelay test app are staged to npm `latest` defaults without writing app files or refreshing lockfiles. It still emits prepare-mode registry warnings until the strict post-publish `ecosystem:sync` / `release:check-npm-latest` gate proves all four npm dist-tags equal `0.20.2`.
- [x] **4.151** Release workflow npm-latest checker reuse — after publishing or confirming immutable npm tarballs, `release-surfaces.yml` now calls `npm run release:check-npm-latest -- --expected-version <semver>` before metadata/app-consumer sync. This keeps the live npm dist-tag proof shared between local operators, ecosystem sync, and CI instead of relying only on inline workflow shell checks.

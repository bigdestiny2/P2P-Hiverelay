...
- [x] **4.39** Public peer list bounds — legacy `/peers`, Node `/api/peers`, and Bare `/api/peers` now share `api-peer-state.js` with a 1000-entry public cap, total/truncated metadata, and malformed peer metadata redaction.
...
- [x] **4.43** Reputation read boundary — public `/api/reputation` and `/api/reputation/:pubkey` now delegate through `api-reputation-read.js`, cap leaderboard rows at 100, return shaped public records, and reuse the same sanitizer for `/api/peers` reputation decorations.
...
| Keep `@noble/secp256k1` + `@noble/hashes` in core for PVSS blind custody; remove unused core-level `@noble/curves` and leave curve-specific Ed25519 usage in `p2p-hiveservices` | Audited | Smaller standalone core package without weakening PVSS/services crypto boundaries |

...
**Date:** 2026-07-31 · **Status:** living operator/product plan · **Baseline:** fleet canary `v0.25.0-rc.9` · stable `v0.24.3` · signed `v0.25.0-rc.8` preserved as failed prerelease · signed `v0.25.0-rc.9` live on the canary cohort · see [FLEET-PLAN-2026-07-31](./FLEET-PLAN-2026-07-31.md)
...
| Item | State |
|------|--------|
| Fleet channel | **canary `v0.25.0-rc.9`** · **stable `v0.24.3`** · held `v0.24.3` |
| Release candidate | Signed `v0.25.0-rc.8` is immutable but failed the exact tagged-source workflow on two portable test-fixture assertions (3568/3570 tests; runtime remained fail-closed). Its successor, signed `v0.25.0-rc.9` (`46d2d7a`), is **cut, published, and live on canary** (2026-07-31): GHCR multi-arch image cosign-signed and smoke-passed, all four npm packages at `next=0.25.0-rc.9` with `latest` untouched, utah and dubai-2gb converged healthily via the gated updater. The pipeline took five infrastructure repairs to complete (npm publish path, store token, store-sync tolerance, deleted upstream git deps, store validation) — all merged; see [FLEET-PLAN-2026-07-31](./FLEET-PLAN-2026-07-31.md) |

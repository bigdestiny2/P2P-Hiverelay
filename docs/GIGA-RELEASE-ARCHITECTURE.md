...
3. **Adversarial/privacy gates** — fail-closed downgrade suites ✓, redaction audit sweeps on public payloads ✓, orphan-pin (403 UNAUTHORIZED_PIN) ✓ and cross-namespace replay rejection ✓ verified 2026-07-18. **Negative-probe: SHIPPED 2026-07-19** — when restricted discovery is configured, the health gate runs a bogus-credential probe after the positive self-probe; an accepted unauthorized connection drops health to DEGRADED (`restricted-discovery-fail-open`) instead of falsely reporting READY (15/15 tor-transport incl. 3 new probe tests).
4. **Release-gate holdouts** — Linux Phase-0 rerun (durability/throughput; harness portable at `00-core/wal-phase0-evidence-2026-07-13/`), app-origin gateway G7–G13 operator/fleet evidence, 100 MB bulk-over-onion measurement. **Resolved since:** P1×P2 atomic-staging lock narrowing ✓ (§5, 34→140 PUTs/s), storage-startup-rollback corestore-7 teardown drain ✓ (`f0676b8`, three root causes fixed), all four pre-existing v1-tip failures ✓ (blind-protocol-vectors, vnext-program-state, vnext-protocol-remediation un-staled; blind-client-late-app rewired to the staged CELL.PUT path `be53898f`). **Wire families:** DESCRIBE + CELL + INBOX ✓ (`2854f9c4`) + CORE unary (MIRROR/PROVE) ✓ (`0dfcb140`) assembled; **owner decisions remaining**: FORWARD (V-6 route budget/acyclic route class — design-blocked per the CR audit), CORE.OPEN_REPLICATION stream path (native descriptor topology, authenticated parent session, upstream signed-head proof authority). **Line status 2026-07-19: 356/356 unit + 45/45 integration green.**

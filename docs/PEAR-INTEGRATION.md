...
p2pbuilders now points at the local Hiverelay `0.24.0` workspace packages and
...
`--use-hiverelay` bridge also points at the local `0.24.0` core/client packages
and dynamically imports the split ESM client while preserving the native relay
default. PearPaste and anonGPT now also point at the local `0.24.0` Hiverelay

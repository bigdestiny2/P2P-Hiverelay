...
| `node --test test/unit/release-blockers-check.test.js test/unit/startos-04-release-evidence.test.js test/unit/release-startos-04-workflow.test.js` | Pass | Covers the closure board, source-built-only StartOS child authority, compare-only public Release pairs, explicit rejection of offline JSON-only closure authority, exact live GitHub Release identity/prerelease policy and asset ids/digests, exact child run attempt and workflow identity, exact REST artifact id/ZIP size/digest/inventory plus terminal revalidation, artifact-byte comparison, adversarial run/artifact substitution, and bounded live-command timeout behavior. |

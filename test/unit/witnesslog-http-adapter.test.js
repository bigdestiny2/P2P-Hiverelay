...
test('witnesslog http adapter: SSE keepalive ping honors the backpressure/teardown gate (v0.21.1 audit)', async (t) => {
  const ctx = createCtx()
  ctx.ssePingMs = 5
  const req = fakeReq('GET', '/api/witness/events?stream=1', null, { accept: 'text/event-stream' })
  const res = fakeRes()
  const handled = await handleWitnessLogRoute(req, res, ctx)
  t.is(handled, true)

...
test('StartOS 0.4 dispatch is source ordered and exact-tag checked before keys', async (t) => {
  ...
  t.ok(workflow.includes('--attempt "$HIVERELAY_RELEASE_SURFACES_RUN_ATTEMPT"'))
  t.ok(workflow.includes('--json databaseId,attempt,url,workflowName,headSha,headBranch,event,jobs'))
  t.ok(workflow.includes('String(run.attempt) !== expectedAttempt'))

...
class FakeControl extends EventEmitter {
  constructor ({ version = '0.4.9.6', failSetConf = false, failAddOnionAt = [], failDelOnionAt = [], serviceIds = [] } = {}) {
    super()
    this.version = version
    this.failSetConf = failSetConf
    this.failAddOnionAt = new Set(failAddOnionAt)
    this.failDelOnionAt = new Set(failDelOnionAt)
    this.serviceIds = serviceIds
    this.addOnionCount = 0
    this.delOnionCount = 0
    this.commands = []
  ...
  cmd (command, _timeout) {
    ...
    if (head === 'DEL_ONION') {
      this.delOnionCount++
      return this.failDelOnionAt.has(this.delOnionCount)
        ? Promise.reject(new Error('injected DEL_ONION failure'))
        : Promise.resolve('250 OK')
    }
    if (head === 'ADD_ONION') {
      this.addOnionCount++
      ...
      const serviceId = this.serviceIds[this.addOnionCount - 1] || SERVICE_ID
...
function fakeRosterTimers () {
  const timers = []
  const set = (fn, delay) => {
    const timer = { fn, delay, cleared: false, fired: false, unref () {} }
    timers.push(timer)
    return timer
  }
  const clear = (timer) => {
  ...
  const next = () => timers.find((timer) => !timer.cleared && !timer.fired)
  const fire = async (timer = next()) => {
    timer.fired = true
    await timer.fn()
  }
  return { set, clear, next, fire }
}
...
test('double identity mismatch fails closed and cannot become the next canonical service', async (t) => {
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  const wrongServiceId = 'c'.repeat(56)
  const first = new FakeControl({ serviceIds: [SERVICE_ID, wrongServiceId, wrongServiceId] })
  ...
  const tt = new TorTransport({
    ...
    localPort: 9100,
    keyFile: path.join(dir, 'hs-key.blob'),
    _controlFactory: () => controls.shift()
  })
  await tt.start()
  ...
  t.ok(caught)
  t.ok(/onion address changed across roster rebuild/.test(caught.message))
  t.ok(caught.rollbackError)
  t.is(first.destroyed, true)
  t.is(tt.running, false)
  t.is(tt.health, TorHealth.DEGRADED)
  t.is(tt.onionAddress, null)
  t.is(tt.serviceId, null)

  await tt.addAuthClient(BOB_PUB)
  await tt.start()
  const restarted = second.commands.find((command) => command.startsWith('ADD_ONION'))
  t.ok(restarted.includes(KEY_BLOB))
  t.ok(restarted.includes('ClientAuthV3=' + BOB_PUB))
  t.is(tt.onionAddress, SERVICE_ID + '.onion', 'restart restores the key-derived canonical identity')
  await tt.stop()
})
...
test('roster save failure plus rollback deletion failure tears the service down', async (t) => {
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  const { control, factory } = fakeFactory({})
  const rosterFile = path.join(dir, 'auth-roster.json')
  ...
  tt._roster.save = async () => { throw new Error('injected roster save failure') }
  control.failDelOnionAt.add(control.delOnionCount + 2)
  ...
  t.ok(caught)
  t.ok(/injected roster save failure/.test(caught.message))
  t.ok(caught.rollbackError)
  t.ok(/injected DEL_ONION failure/.test(caught.rollbackError.message))
  t.alike(tt.listAuthClients(), [ALICE_PUB], 'rejected mutation is restored in memory')
  const persisted = JSON.parse(fs.readFileSync(rosterFile, 'utf8'))
  t.alike(persisted.keys.filter((entry) => !entry.revokedAtMs).map((entry) => entry.pub), [ALICE_PUB])
  t.is(control.destroyed, true, 'closing the owning control tears down the untrusted live service')
  t.is(tt.running, false)
  t.is(tt.health, TorHealth.DEGRADED)
  t.is(tt.onionAddress, null)
  t.is(tt.serviceId, null)
...
test('stop, offline roster mutation, and restart serialize behind an active rebuild', async (t) => {
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  ...
  const realCmd = first.cmd.bind(first)
  let blockNextDel = true
  first.cmd = (command, timeout) => {
    const response = realCmd(command, timeout)
    if (blockNextDel && command.startsWith('DEL_ONION')) {
      blockNextDel = false
      entered.resolve()
  ...
  const mutation = tt.addAuthClient(ALICE_PUB)
  ...
  tt.once('stopped', () => {
    stoppedSnapshot = {
      running: tt.running,
      health: tt.health,
      onionAddress: tt.onionAddress,
      serviceId: tt.serviceId
    }
  })
  let stopResolved = false
  const stop = tt.stop().then(() => { stopResolved = true })
  const offlineMutation = tt.addAuthClient(BOB_PUB)
  const restart = tt.start()

  await Promise.resolve()
  t.is(stopResolved, false, 'stop waits behind the active roster transaction')

  release.resolve()
  await Promise.all([mutation, stop, offlineMutation, restart])

  t.alike(stoppedSnapshot, {
    running: false,
    health: TorHealth.DISABLED,
    onionAddress: null,
    serviceId: null
  })
  t.is(first.destroyed, true)
  t.is(
    first.commands.filter((command) => command.startsWith('ADD_ONION')).length,
    2,
    'offline mutation did not rebuild through the stopped generation'
  )
  const restarted = second.commands.find((command) => command.startsWith('ADD_ONION'))
  t.ok(restarted.includes('ClientAuthV3=' + ALICE_PUB))
  t.ok(restarted.includes('ClientAuthV3=' + BOB_PUB))
  t.is(tt.running, true)
  t.is(tt.health, TorHealth.KEY_LOADED)
  first.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' stale-1')
  first.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' stale-2')
  t.is(tt.health, TorHealth.KEY_LOADED, 'events from the stopped control generation are ignored')
  second.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' fresh-1')
  second.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' fresh-2')
  t.is(tt.health, TorHealth.READY)
  await tt.stop()
})

test('roster rebuild withdraws signed onion capability before DEL_ONION settles', async (t) => {
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  const { control, factory } = fakeFactory({})
  const tt = new TorTransport({
  ...
  let blockNextDel = true
  let blockNextBarrier = true
  control.cmd = (command, timeout) => {
    const response = realCmd(command, timeout)
    if (blockNextDel && command.startsWith('DEL_ONION')) {
      blockNextDel = false
      entered.resolve()
    ...
    if (blockNextBarrier && command.startsWith('SETEVENTS')) {
      blockNextBarrier = false
      barrierEntered.resolve()
      return releaseBarrier.promise.then(() => response)
    }
    return response
  }
  ...
  const mutation = tt.addAuthClient(ALICE_PUB)
  await entered.promise
  t.is(tt.health, TorHealth.KEY_LOADED, 'readiness is withdrawn before Tor mutates the service')

  const doc = buildCapabilityDoc({
    relay: { config: { tor: {} }, torTransport: tt },
    attestedAt: 1784323200000
  })
  t.ok(doc.supported_transports.includes('tor'))
  t.absent('privacyTransports' in doc)
  t.is(doc.onionGatewayUrl, null)
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' stale-del-1')
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' stale-del-2')
  t.is(tt.health, TorHealth.KEY_LOADED)
  t.is(tt._descriptorUploads, 0)

  release.resolve()
  await barrierEntered.promise
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' stale-barrier-1')
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' stale-barrier-2')
  t.is(tt.health, TorHealth.KEY_LOADED, 'old events stay fenced through the control ordering barrier')
  t.is(tt._descriptorUploads, 0)
...
test('probe result from the previous service generation cannot restore readiness', async (t) => {
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  const { control, factory } = fakeFactory({})
  ...
  await tt.start()
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' old-1')
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' old-2')
  ...
  t.is(tt.health, TorHealth.KEY_LOADED, 'resolved old-generation probe is discarded')
  t.is(tt._descriptorUploads, 0)
  await tt.stop()
})

test('runtime roster expiry removes authorization and retries a failed replacement', async (t) => {
  const nowBase = 1784323200000
  let now = nowBase
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  const timers = fakeRosterTimers()
  const { control, factory } = fakeFactory({})
  const tt = new TorTransport({
    socksPort,
    localPort: 9100,
    ...
    _controlFactory: factory,
    _now: () => now,
    _setRosterTimer: timers.set,
    _clearRosterTimer: timers.clear,
    _rosterExpiryRetryMs: 250
  })
  await tt.start()
  await tt.addAuthClient(ALICE_PUB, { expiresAtMs: nowBase + 1000 })

  const expiry = timers.next()
  t.is(expiry.delay, 1000)
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' first')
  control.emit('event', 'HS_DESC UPLOADED ' + SERVICE_ID + ' second')
  t.is(tt.health, TorHealth.READY)
  ...
  control.failAddOnionAt.add(control.addOnionCount + 1)
  now = nowBase + 1000
  await timers.fire(expiry)
  await tt._operationQueue

  t.alike(tt.listAuthClients(), [], 'expired authorization stays removed after daemon failure')
  t.is(tt.health, TorHealth.DEGRADED)
  t.is(tt._serviceActive, false, 'successful DEL plus failed ADD leaves no live onion service')

  const retry = timers.next()
  t.is(retry.delay, 250)
  await timers.fire(retry)
  await tt._operationQueue

  const rebuilt = control.commands.filter((command) => command.startsWith('ADD_ONION')).pop()
  t.absent(rebuilt.includes('ClientAuthV3=' + ALICE_PUB))
  t.ok(rebuilt.includes('Flags=V3Auth'))
  t.ok(/ClientAuthV3=[a-z2-7]{52}/.test(rebuilt), 'empty restricted roster uses its guard key')
  t.is(tt._serviceActive, true)
  t.is(tt.health, TorHealth.KEY_LOADED)
  await tt.stop()
})

test('expiry retry preserves the canonical onion identity after a mismatched response', async (t) => {
  const nowBase = 1784323200000
  let now = nowBase
  const wrongServiceId = 'c'.repeat(56)
  const socksPort = await fakeSocks(t)
  ...
  const timers = fakeRosterTimers()
  const { control, factory } = fakeFactory({
    serviceIds: [SERVICE_ID, SERVICE_ID, wrongServiceId, SERVICE_ID]
  })
  const tt = new TorTransport({
    socksPort,
    localPort: 9100,
  ...
  t.is(tt.onionAddress, SERVICE_ID + '.onion', 'rejected response cannot replace the expected identity')
  t.is(tt.serviceId, SERVICE_ID)
  const cleanup = control.commands.filter((command) => command === 'DEL_ONION ' + wrongServiceId)
  t.is(cleanup.length, 1, 'mismatched service is explicitly removed')
...
test('expired roster tombstone survives clock rollback, mutation, and restart', async (t) => {
  const nowBase = 1784323200000
  let now = nowBase
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  ...
  now = nowBase + 1000
  await timers.fire(timers.next())
  await tt._operationQueue
  let persisted = JSON.parse(fs.readFileSync(rosterFile, 'utf8'))
  t.is(persisted.keys.find((entry) => entry.pub === ALICE_PUB).revokedAtMs, now)

  now = nowBase
  await tt.addAuthClient(BOB_PUB, { expiresAtMs: nowBase + 5000 })
  let live = first.commands.filter((command) => command.startsWith('ADD_ONION')).pop()
  t.absent(live.includes('ClientAuthV3=' + ALICE_PUB), 'clock rollback cannot revive the tombstoned key')
  t.ok(live.includes('ClientAuthV3=' + BOB_PUB))

  await tt.stop()
  await tt.start()
  live = second.commands.find((command) => command.startsWith('ADD_ONION'))
  t.absent(live.includes('ClientAuthV3=' + ALICE_PUB), 'restart preserves monotonic expiry')
  t.ok(live.includes('ClientAuthV3=' + BOB_PUB))
  persisted = JSON.parse(fs.readFileSync(rosterFile, 'utf8'))
  t.ok(persisted.keys.find((entry) => entry.pub === ALICE_PUB).revokedAtMs !== null)
  await tt.stop()
})

test('startup durably tombstones entries first observed after expiry', async (t) => {
  const nowBase = 1784323200000
  let now = nowBase
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  ...
  fs.writeFileSync(rosterFile, JSON.stringify({
    ...
    keys: [{
      ...
      addedAtMs: nowBase - 2000,
      expiresAtMs: nowBase - 1000,
      revokedAtMs: null
    }]
  }))
  const first = new FakeControl()
  ...
  let persisted = JSON.parse(fs.readFileSync(rosterFile, 'utf8'))
  t.is(persisted.keys[0].revokedAtMs, nowBase)
  let live = first.commands.find((command) => command.startsWith('ADD_ONION'))
  t.absent(live.includes('ClientAuthV3=' + ALICE_PUB))

  now = nowBase - 1500
  await tt.addAuthClient(BOB_PUB, { expiresAtMs: nowBase + 5000 })
  live = first.commands.filter((command) => command.startsWith('ADD_ONION')).pop()
  t.absent(live.includes('ClientAuthV3=' + ALICE_PUB), 'rewind plus mutation cannot revive startup expiry')
  await tt.stop()
  await tt.start()
  live = second.commands.find((command) => command.startsWith('ADD_ONION'))
  t.absent(live.includes('ClientAuthV3=' + ALICE_PUB))
  t.ok(live.includes('ClientAuthV3=' + BOB_PUB))
  persisted = JSON.parse(fs.readFileSync(rosterFile, 'utf8'))
  t.is(persisted.keys.find((entry) => entry.pub === ALICE_PUB).revokedAtMs, nowBase)
  await tt.stop()
})

test('short expiry recheck catches a forward wall-clock correction', async (t) => {
  const nowBase = 1784323200000
  let now = nowBase
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  ...
  await tt.start()
  await tt.addAuthClient(ALICE_PUB, { expiresAtMs: nowBase + 24 * 60 * 60 * 1000 })
  const recheck = timers.next()
  t.is(recheck.delay, 60_000, 'long leases are rechecked against wall clock once per minute')

  now = nowBase + 2 * 24 * 60 * 60 * 1000
  await timers.fire(recheck)
  await tt._operationQueue
  t.alike(tt.listAuthClients(), [])
  await tt.stop()
})

test('runtime expiry closes the control-owned service when DEL_ONION fails', async (t) => {
  const nowBase = 1784323200000
  let now = nowBase
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  ...
  await tt.addAuthClient(ALICE_PUB, { expiresAtMs: nowBase + 1000 })
  const expiry = timers.next()
  first.failDelOnionAt.add(first.delOnionCount + 1)

  now = nowBase + 1000
  await timers.fire(expiry)
  await tt._operationQueue

  t.alike(tt.listAuthClients(), [], 'expiry remains committed in memory')
  t.is(first.destroyed, true, 'closing the owning control connection tears down an unconfirmed service')
  t.is(tt.running, false)
  t.is(tt.health, TorHealth.DEGRADED)
  t.is(tt.onionAddress, null)
  t.is(tt.serviceId, null)
  ...
  await tt.start()
  const restarted = second.commands.find((command) => command.startsWith('ADD_ONION'))
  t.absent(restarted.includes('ClientAuthV3=' + ALICE_PUB))
  t.ok(restarted.includes('Flags=V3Auth'))
  t.is(tt.running, true)
  await tt.stop()
})

test('runtime expiry closes the live service when its persistent key becomes unreadable', async (t) => {
  const nowBase = 1784323200000
  let now = nowBase
  const socksPort = await fakeSocks(t)
  const dir = tmpdir(t)
  ...
  tt.on('health', () => { throw new Error('injected health observer failure') })

  now = nowBase + 1000
  await timers.fire(expiry)
  await tt._operationQueue

  t.alike(tt.listAuthClients(), [], 'expired authorization remains removed')
  t.is(control.destroyed, true, 'unrebuildable control-owned service is torn down')
  t.is(tt.running, false)
  t.is(tt.health, TorHealth.DEGRADED)
  t.is(tt.onionAddress, null)
  t.is(tt.serviceId, null)
